...
def salvar_dados():
    conexao = sqlite3.connect("dados_cadastro.db")
    terminal_sql = conexao.cursor()
    terminal_sql.execute(f"INSERT INTO pessoas (nomes, preco, desc) VALUES ('{Colocar_Nome_Cadastrar.get()}',"
...
def ler_dados_lista_editar():
    ...
    conexao = sqlite3.connect("dados_cadastro.db")
    terminal_sql = conexao.cursor()
    terminal_sql.execute(f"SELECT nomes FROM pessoas")
...
def ler_dados_lista_saida():
    global items, check_var , valor_checkbox
    conexao = sqlite3.connect("dados_cadastro.db")
    terminal_sql = conexao.cursor()
    terminal_sql.execute(f"SELECT nomes FROM pessoas")
    items = terminal_sql.fetchall()
...
def deletar_items(nome_produto):
    conexao = sqlite3.connect("dados_cadastro.db")
    terminal_sql = conexao.cursor()
    terminal_sql.execute(F"DELETE FROM pessoas WHERE nomes = '{nome_produto}'")
    conexao.commit()
    conexao.close()
